...
public class Board {
    ArrayList<Pair<Integer,Integer>>positonCoordinates;

    ArrayList<Integer>snakeLadderPosition;


    public Board(){
        positonCoordinates=new ArrayList<>();
       populatePositonCoordinates();
       populateSnakeLadder();
    }
    private void populatePositonCoordinates(){
        positonCoordinates.add(new Pair<>(0,0));
        for (int i = 0; i < SnakeLadder.height; i++) {
            for (int j = 0; j < SnakeLadder.width; j++) {
                ...
                positonCoordinates.add(new Pair<>(xCord,yCord));
    ...
    public  int getNewPosition(int currentPosition){
      ...
      int getXCoordinates(int position){
        if(position>=1 && position<=100)
            return positonCoordinates.get(position).getKey();
            return -1;

      }
    int getYCoordinates(int position){
        if(position>=1 && position<=100)
            return positonCoordinates.get(position).getValue();
        return -1;

    }
    //public static void main(String[] args) {
     //   Board board=new Board();
        //for (int i = 0; i <board.positonCoordinates.size(); i++) {
     //   System.out.println(i + " $ x :"+board.positonCoordinates.get(i).getKey()+
           //     " y :"+board.positonCoordinates.get(i).getValue()
      //  );
       // }
    //}
}
